...
use cosmwasm_std::{
    to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdError, StdResult, Uint128, IbcMsg, IbcTimeout, CosmosMsg, WasmMsg, Coin
};
use injective_cosmwasm::{
    create_spot_market_order_msg, create_withdraw_msg,create_batch_update_orders_msg, 
    get_default_subaccount_id_for_checked_address, InjectiveMsgWrapper, InjectiveQuerier, OrderType, SpotOrder, MarketId, DerivativeOrder, SubaccountId, create_derivative_market_order_msg
};
...
use cw20::{
    ...
    MarketingInfoResponse, MinterResponse, TokenInfoResponse, Cw20ReceiveMsg, ExchangeRateResponse
};
...
use crate::state::{
    ...
    TOKEN_INFO, CONFIG_STATE, SWAP_OPERATION_STATE, SwapCacheState, STATE, ContractConfigState
};
...
pub fn instantiate(
    ...
    let state = ContractConfigState {
        ...
        genesis_block_number: _env.block.height,
        savings_rate: 1,
        owner: _info.sender,
        block_generation_time_ms: msg.block_generation_time_ms,
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Deposit { spot_quantity, derivative_quantity, spot_price, derivative_price, spot_market_id, derivative_market_id, channel_id} => execute_deposit(deps, env, info, spot_quantity, derivative_quantity, spot_price, derivative_price, spot_market_id, derivative_market_id,channel_id),
        ExecuteMsg::Transfer { recipient, amount } => {
            execute_transfer(deps, env, info, recipient, amount)
        }
        ExecuteMsg::UpdateGenesisBlockNumber { block_number } => execute_update_genesis_block_number( deps, env, info, block_number),
        ExecuteMsg::UpdateSavingsRate { rate } => execute_update_savings_rate( deps, env, info, rate),
...
pub fn execute_update_genesis_block_number(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    block_number: u64,
) -> Result<Response<InjectiveMsgWrapper>, ContractError> {
    CONFIG_STATE.update(deps.storage, |mut config| -> Result<_, ContractError> {
        if info.sender != config.owner {
            return Err(ContractError::Unauthorized {});
        }
        config.genesis_block_number = block_number;
        
        Ok(config)
    })?;
    Ok(Response::new().add_attribute("action", "Genesis block number updated"))
}

pub fn execute_update_savings_rate(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    rate: u64,
    ...
    CONFIG_STATE.update(deps.storage, |mut config| -> Result<_, ContractError> {
        ...
        config.savings_rate = rate;
        
        Ok(config)
    })?;
    Ok(Response::new().add_attribute("action", "Savings rate updated"))
}
...
pub fn execute_deposit(
    ...
    env: Env,
    info: MessageInfo,
    spot_quantity: FPDecimal,
    derivative_quantity: FPDecimal,
    spot_price: FPDecimal,
    derivative_price: FPDecimal,
    spot_market_id: MarketId,
    derivative_market_id: MarketId,
    channel_id: String,
) -> Result<Response<InjectiveMsgWrapper>, ContractError> {
    ...
    if info.funds[0].denom != "peggy0x87aB3B4C8661e07D6372361211B96ed4Dc36B1B5".to_string() {
        return Err(ContractError::CustomError {
            val: "You should deposit only USDT!".to_string(),
        });
    }   

    let calculated_amount_musd = info.funds[0].amount.checked_mul(config.savings_rate.into()).unwrap();

    let execute_msg = WasmMsg::Execute {
        contract_addr: contract.clone().to_string(),
        msg: to_binary(&ExecuteMsg::Mint { recipient: info.sender.to_string(), amount: calculated_amount_musd } )?,
        funds: vec![],
    };
    ...
    let order = SpotOrder::new(
        ...
        spot_quantity,
        OrderType::BuyAtomic,
        &spot_market_id.clone(),
        subaccount_id.clone(),
        Some(contract.clone()),
    );

    let derivative_order = DerivativeOrder::new(
        derivative_price,
        derivative_price,
        derivative_price*derivative_price,
        OrderType::SellAtomic,
        derivative_market_id.clone(),
        subaccount_id.clone(),
        Some(contract.clone()),
    );

    // IBC transfer is closed on Testnet
    // timeout is in nanoseconds
    let timeout = env.block.time.plus_seconds(5);
    let ibc_transfer_msg = IbcMsg::Transfer{ 
        channel_id: channel_id, 
        to_address: "cosmos1m7n24qydy77e24pcadlshpsq5j5mv83xyvarsn".to_string(),
        amount: Coin { 
                denom: "inj".to_string(),
    ...
    let spot_order_msg = create_spot_market_order_msg(contract.clone(), order);
    let derivative_order_msg = create_derivative_market_order_msg(contract.clone(), derivative_order);

    let response = Response::new()
        .add_message(spot_order_msg)
        .add_message(derivative_order_msg)
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::ExchangeRate {  } => to_binary(&query_exchange_rate(deps, _env)?),
...
pub fn query_exchange_rate(deps: Deps, _env: Env) -> StdResult<ExchangeRateResponse> {
    let config = CONFIG_STATE.load(deps.storage)?;
    let MILLISECONDS_IN_YEAR_: u64 = 31_536_000_000;

    if (_env.block.height.clone() < config.genesis_block_number){
        return Ok(ExchangeRateResponse { rate: 1 } )
    }   
    let rate = ((_env.block.height - config.genesis_block_number)/(MILLISECONDS_IN_YEAR_ / config.block_generation_time_ms))*config.savings_rate;
    Ok(ExchangeRateResponse { rate: rate} )
}
...
#[cfg_attr(not(feature = "library"), entry_point)]
pub fn migrate(deps: DepsMut, _env: Env, _msg: MigrateMsg) -> Result<Response<InjectiveMsgWrapper>, ContractError> {
